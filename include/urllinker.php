 ...
 */
function htmlEscapeAndLinkUrls($text, $target='')
{
	$rexScheme    = 'https?://';
    ...
    while (preg_match($rexUrlLinker, $text, $match, PREG_OFFSET_CAPTURE, $position))
    {
        ...
        if (preg_match('{^\.[0-9]{1,3}$}', $tld) || isset($validTlds[$tld]))
        {
            ...
            else
            {
				...
				if (strlen($shortenedPath) > 25) $shortenedPath = substr($shortenedPath, 0, 25).'…';
                $linkText = "$domain$port$shortenedPath";
            }

			$targetCode = strlen($target) ? 'target="'.$target.'"' : '';
            $linkHtml = '<a '.$targetCode.'href="' . htmlspecialchars($completeUrl) . '">'
 ...
 */
function linkUrlsInTrustedHtml($html, $target='')
{
    ...
    while (true)
    {
        ...
        if (!$insideAnchorTag) $text = htmlEscapeAndLinkUrls($text, $target);
