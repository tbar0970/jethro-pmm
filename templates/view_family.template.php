...
if ($GLOBALS['user_system']->havePerm(PERM_EDITPERSON)) {
	$links[] = '<a href="?view=_edit_family&familyid='.$family->id.'"><i class="icon-wrench"></i>'._('Edit').'</a>';
	if (Person::allowedToAdd()) {
		// users with group or cong restrictions are not allowed to add persons
		$links[] = '<a href="?view=_add_person_to_family&familyid='.$family->id.'"><i class="icon-plus-sign"></i>'._('Add Member').'</a>';
	}
}
if (!empty($links)) {
	?>
...
<div class="align-right">
	...
	if ($family->getPostalAddress() != '') {
		$links[] = '<a href="?call=envelopes&familyid='.$family->id.'" class="envelope-popup"><i class="icon-envelope"></i>'._('Print Envelope').'</a>';
	}
	$all_emails = $family->getAllEmailAddrs();
	if (!empty($all_emails)) {
		$links[] = '<a href="'.get_email_href($all_emails).'" '.email_link_extras().'><i class="icon-email">@</i>'._('Email All').'</a>';
	}
	echo implode(' &nbsp; ', $links);
	?>
</div>
