    ...
    <header class="bmd-layout-header">
      <div class="navbar navbar-light bg-faded">
        ...
        <ul class="nav navbar-nav">
          <li class="nav-item">
            Jethro PMM - <?php echo SYSTEM_NAME; ?>
          </li>
        </ul>
...
?>
  <div id="my-drawer" class="bmd-layout-drawer">
    <header>
      ...
      <div class="account">
        <span><?php echo $GLOBALS['user_system']->getCurrentUser('first_name').' '.$GLOBALS['user_system']->getCurrentUser('last_name'); ?></span>
        <div class="dropdown">
          ...
          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="nav-account-menu">
            <a class="dropdown-item" href="?view=_edit_me"><i class="material-icons">add</i><?php echo _('Edit me')?></a>
            <a class="dropdown-item" href="./?logout=1" data-method="post"><i class="material-icons">add</i><?php echo _('Log out')?></a>
          </div>
        </div>
        <?php
        if ($GLOBALS['user_system']->getCurrentRestrictions()) {
          ?>
          <span class="restrictions" title=<?php echo _('"This user account can only see persons in certain congregations or groups"')?>> <?php echo _('Restrictions in effect')?> </span>
          <?php
        }
        ?>
      </div>
