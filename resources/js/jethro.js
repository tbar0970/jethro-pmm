/*jslint browser: true, devel: true, continue: true, evil: true, plusplus: true, regexp: true, sloppy: true, vars: true*/
/*global $ */
/*global TBLib */
/*global parseQueryString */
// TODO: more sniffing of the relevant bits (like attendance does)
...
$(document).ready(function () {
  ...
  $('.nav a').each(function () {
    ...
    if (($(this).html() !=='') && ($(this).html().toLowerCase() === 'search')) {
      $(this).on('click', handleSearchLinkClick);
  ...
  $('form.bulk-person-action').submit(function (event) {
    ...
    alert($(this).prop('action').match('send_sms_http$'));
    if ($(this).prop('action').match('send_sms_http$')) {
      ...
      $.ajax({
        ...
        error: function (jqXHR, status, error) {
          console.log('Error sending SMS: ', status, error);
          var statusBtn = $(this).find('.bulk-sms-status');
          statusBtn.unbind('click');
          statusBtn.on('click', function (event) { event.preventDefault(); alert(error); });
          statusBtn.toggleClass('fade');
        },
        success: function (data) {
          ...
          if (data.blank !== undefined) { blankCount = data.blank.count; }
          if (data.rawresponse !== undefined) { rawresponse = data.rawresponse; }
          if (data.error !== undefined) { error = data.error; }
          ...
          if (failedCount > 0) {
            failDiv.append("<h4>Failed Recipients</h4><p style='clear:both'>SMS sending failed for " + failedCount + ' recipients.</p>');
          }
          if (blankCount > 0) {
          ...
          failDiv.append(data);
          if (successCount !== $("input[name='personid[]']:checked").length) {
            statusBtn.unbind('click');
            statusBtn.on('click', function (event) { event.preventDefault(); alert(error); });
            statusBtn.addClass('fade');
          } else {
            statusBtn.unbind('click');
            statusBtn.removeClass('fade');
            $('#smshttp').toggle();
            alert('Message sent');
          }
...
function handleSearchLinkClick()
{
  ...
  if (url!==undefined) {
    var queryVars = parseQueryString(url.substr(url.indexOf('?') + 1));
...
$(document).ready(function () {
  $('input[data-set-form-target], button[data-set-form-target]').click(function () {
    switch ($(this).attr('data-set-form-target')) {
      case 'envelope':
        var envelopeWindow = window.open('', 'envelopes', 'height=320,width=500,location=no,menubar=no,titlebar=no,toolbar=no,resizable=yes,statusbar=no');
        if (envelopeWindow) {
...
{
  var family_name = $(this).val();
  $('form#add-family .last_name input').each(function () {
    if ($(this).val() === '') {
      $(this).val(family_name);//.replace("'", "\\'"));
    }
  });
}
